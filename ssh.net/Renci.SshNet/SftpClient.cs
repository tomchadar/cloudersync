...
namespace Renci.SshNet
{
    ...
    public class SftpClient : BaseClient
    {
        ...
        /// </remarks>
        public void UploadFile(Stream input, string path, CancellationToken cancelToken,Action<ulong, ulong> uploadCallback = null)
        {
            UploadFile(input, path, true, cancelToken,uploadCallback);
        }
        ...
        /// Method calls made by this method to <paramref name="input" />, may under certain conditions result in exceptions thrown by the stream.
        /// </remarks>
        public void UploadFile(Stream input, string path, bool canOverride, CancellationToken cancelToken, Action<ulong, ulong> uploadCallback = null)
        {
            CheckDisposed();
            ...
            InternalUploadFile(input, path, flags, cancelToken, null, uploadCallback);
        }
        ...
        /// </para>
        /// </remarks>
        public IAsyncResult BeginUploadFile(Stream input, string path, AsyncCallback asyncCallback, object state, Action<ulong, ulong> uploadCallback = null)
        {
        ...
        /// </para>
        /// </remarks>
        public IAsyncResult BeginUploadFile(Stream input, string path, bool canOverride, AsyncCallback asyncCallback, object state, Action<ulong, ulong> uploadCallback = null)
        {
            CheckDisposed();
            ulong bufSize = (ulong)input.Length;
        ...
        /// <param name="cancelToken">CaancellationToken</param>
        /// <returns>
        ...
        public IEnumerable<FileInfo> SynchronizeDirectories(string sourcePath, string destinationPath, string searchPattern, CancellationToken cancelToken)
        {
            if (sourcePath == null)
            ...
            return InternalSynchronizeDirectories(sourcePath, destinationPath, searchPattern, null,cancelToken);
        }
        ...
        /// <exception cref="ArgumentNullException"><paramref name="sourcePath"/> is <c>null</c>.</exception>
        /// <exception cref="ArgumentException"><paramref name="destinationPath"/> is <c>null</c> or contains only whitespace.</exception>
        public IAsyncResult BeginSynchronizeDirectories(string sourcePath, string destinationPath, string searchPattern, CancellationToken cancelToken, AsyncCallback asyncCallback, object state)
        {
            if (sourcePath == null)
                throw new ArgumentNullException("sourcePath");
            if (destinationPath.IsNullOrWhiteSpace())
            ...
            ThreadAbstraction.ExecuteThread(() =>
            {
                try
                {
                    var result = InternalSynchronizeDirectories(sourcePath, destinationPath, searchPattern,asyncResult, cancelToken);
        ...
        private IEnumerable<FileInfo> InternalSynchronizeDirectories(string sourcePath, string destinationPath, string searchPattern, SftpSynchronizeDirectoriesAsyncResult asynchResult,CancellationToken cancelToken)
        {
            ...
            foreach (var localFile in sourceFiles)
            {
                ...
                if (isDifferent)
                {
                    ...
                    try
                    {
                        ...
                        {
                            InternalUploadFile(file, remoteFileName, uploadFlag, cancelToken, null, null);
                        }
        ...
        /// <exception cref="ArgumentException"><paramref name="path" /> is <b>null</b> or contains whitespace.</exception>
        /// <exception cref="SshConnectionException">Client not connected.</exception>
        private void InternalUploadFile(Stream input, string path, Flags flags, CancellationToken cancelToken, SftpUploadAsyncResult asyncResult, Action<ulong, ulong> uploadCallback)
        {
            if (input == null)
                throw new ArgumentNullException("input");
                    ...
                    _sftpSession.RequestWrite(handle, offset, buffer, bytesRead, null, s =>
                        {
                            if (s.StatusCode == StatusCodes.Ok)
                            {
                                ...
                                {
                                    //  Execute callback on different thread
                                    ThreadAbstraction.ExecuteThread(() => uploadCallback(writtenBytes, bufSize));
                                }
                            }
                        });
